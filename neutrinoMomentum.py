''' 
Taken from "Analytic solutions for neutrino momenta in decay of top quarks" by Burton A. Betchart, Regina Demina, Amnon Harel
https://arxiv.org/pdf/1305.1878
'''
import numpy as np
import ROOT as r
import math
...
def cofactor(A, ij):
    ...
    i, j = ij  # Unpack the tuple inside the function
...
def intersections_ellipse_line(ellipse, line, zero=1e-12):
    ...
    sols = sorted([(v.real / v[2].real, 
                   ...
                   for v in V.T], 
                  key=lambda s_k: s_k[1])[:2]  # Fix lambda unpacking issue
...
if __name__ in "__main__":
    ...
    b_vec = r.TLorentzVector()
    mu_vec = r.TLorentzVector()
    met = r.TLorentzVector()

    met.SetPtEtaPhiM(39.11084702421775,0, 0.8734979753423628,0)


    b_vec.SetPtEtaPhiM(77.41118922, -0.00711823, -2.5634766, 12.44741588)

    mu_vec.SetPtEtaPhiM(71.625, -0.59814453, 0.38867188, 0.105658)

    sol = singleNeutrinoSolution(b=b_vec, mu=mu_vec, metX=met.Px(), metY=met.Py(), sigma2=[[0.1,0],[0,0.1]])

    print("solution", sol.nu)
    print("chi2", sol.chi2)
